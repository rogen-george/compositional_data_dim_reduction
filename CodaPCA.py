...
class CodaPCA( BaseDR ):
    ...
    def transform( self, X, sess=None ):
        ...
        '''
        print ("here doing transformation")
        print ( X.shape )
    ...
    def __rebuild_graph( self, dim_x ):
        ...
        with tf.variable_scope( 'CodaPCA' ):
            ...
            logMeanX = tf.reduce_sum( logX*mask, axis=1, keepdims=True ) / degree
            clrX = logX - logMeanX
            ...
            if self.alg in [ Alg.CODAAE, Alg.CLRAE ]:
                ...
                corrupt_clrX = corrupt_logX - tf.reduce_mean( corrupt_logX, axis=1, keepdims=True )
                corrupt_X = tf.exp( corrupt_logX - tf.reduce_logsumexp( corrupt_logX, axis=1, keepdims=True ) )
            ...
            if self.alg == Alg.SCODAPCA:
                ...
                self.Y -= tf.reduce_sum( mask*self.Y, 1, keepdims=True ) / degree
    ...
    def __fit( self, sess, saver, X, epochs, verbose ):
        ...
        for epoch in range( epochs ):
            ...
            for i, batchidx in enumerate( self.batches( allidx, self.batchsize) ):
                _loss, _ = sess.run( [self.loss,self.train_op],
                                     feed_dict={ self.X: X[batchidx],
