...
def get_data_diet_swap( ):

    data_afr = pd.read_excel("data_afr.xlsx")
    data_aam = pd.read_excel("data_aam.xlsx")

    positive_train = np.array(data_afr[:60])
    negative_train = np.array(data_aam[:60])

    validation_positive = np.array(data_afr[60:80])
    validation_negative = np.array(data_aam[60:80])

    test_positive = np.array(data_afr[80:])
    test_negative = np.array(data_aam[80:100])
